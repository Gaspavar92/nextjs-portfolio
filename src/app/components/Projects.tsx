...
const Projects = () => {
    ...
    const descriptions: { [key: string]: {name: string, description: string, tech: string[]} } = {
        ...
        "world-whereabouts": {
            ...
            description: "Research every country in the world; it leverages an API to find each location.",
            tech: [
                "/react.svg",
                "/javascript.svg",
                "/sass.svg"
        ...
        "ai-travels": {
            name: "AI Travels",
            description: "For your next trip, create an AI-generated itinerary for any place in the world.",
            tech: [
                "/javascript.svg",
                "/html-5.svg",
                "/css-3.svg",
    ...
    return (
            <>
                <div id="projects" className={`${protest.className} project-text section-title md:h-screen h-[60vh] text-[25vw] flex items-center`}>{project.map((letter, index) => {
                    return letter === " " ? <p key={index} className="project-letter inline-block !invisible">-</p> : <p key={index} className="project-letter inline-block">{letter}</p>
                })}</div>
            <div className="projects wrapper">
                <div className="project-grid grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
                    {projects.map((project) => (
                        <div key={project.id} id={project.id} className="project-card h-62 rounded-lg shadow-lg overflow-hidden transform transition relative">
                            ...
                            <div className="project-info p-2 md:p-6 absolute bottom-0 flex flex-col w-full h-full justify-end">
                                ...
                                <p className={`${sono.className} mb-4 text-xs`}>{descriptions[project.name] ? descriptions[project.name].description : "No description"}</p>
                                <div className="flex justify-between items-center text-sm">
                                    <div className="flex space-x-2">
                                        <a href={project.url} target="_blank" rel="noopener noreferrer" aria-label="View live project" className="flex gap-2 hover:underline text-sky-600 cursor-pointer">
                                        ...
                                        <a href={project.build_settings.repo_url} target="_blank" rel="noopener noreferrer" aria-label="View GitHub repository" className="flex gap-2 hover:underline text-sky-600 cursor-pointer">
